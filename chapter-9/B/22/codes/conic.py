...
O=np.array([0,0])    #centre of ellipse
V = np.matrix('9,0;0,16')	
l,P = np.linalg.eigh(V)		#l-eigen_values & P-eigen_vector
f = -144
...
C = np.sqrt(-(l2*((e*e)-1)*(-l2*f)))/(l2*e*((e*e)-1))	#constant
F = (C*e*e*n)/l2			#Focus of ellpise
r = LA.norm(c-F.T)                      #radius of circle   
...
tri_coords = np.vstack((A,B,C,D,F1,F2,O)).T
plt.scatter(tri_coords[0,:], tri_coords[1,:])
vert_labels = ['A','B','C','D','F1','F2','O']
...
#if using termux
plt.savefig('/home/dell/matrix/conic.pdf')
